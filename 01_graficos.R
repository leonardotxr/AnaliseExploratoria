...
ggplot(df, aes(y = tempo_preso)) +
  geom_boxplot(fill = "orange") +
  labs(title = "Boxplot do Tempo Preso", y = "Tempo preso (meses)")
ggsave("figuras/box_tempo_preso.png", width = 6, height = 5)

## Boxplot do score por escolaridade
ggplot(df, aes(x = escolaridade, y = score_periculosidade)) +
  geom_boxplot(fill = "lightgreen") +
  labs(title = "Score por Escolaridade", x = "Escolaridade", y = "Score de Periculosidade") +
  theme(axis.text.x = element_text(angle = 30, hjust = 1))
ggsave("figuras/box_score_por_escolaridade.png", width = 8, height = 5)

## Gráfico de barras para reincidente
ggplot(df, aes(x = reincidente)) +
  geom_bar(fill = "tomato") +
  labs(title = "Frequência de Reincidentes", x = "Reincidente", y = "Contagem")
